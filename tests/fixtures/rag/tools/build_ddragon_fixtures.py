...
# 4자리 ID 아이템만 담고, 6자리 ID 는 다른 모드용 사본으로 보고 뺀다.
# 6자리 중에도 maps["11"] 이 참인 것이 있어 맵 검사만으로는 걸러지지 않는다.
# 예: 322065 는 2065(슈렐리아의 군가) 앞에 32 를 붙인 것이고 이름도 같다. 6자리 440개 중 279개가 이런 모양이다.
# 나머지 161개(예: 663193)는 이름이 같은 짝이 없는데도 같이 뺀다.
# 실제 경기 기록에서 협곡 아이템이 4자리로 오는지는 아직 확인하지 않았다.
# 6자리로 오는 협곡 아이템이 있으면 이 규칙을 고쳐야 한다. (src/rag/DATA_DRAGON.md)
