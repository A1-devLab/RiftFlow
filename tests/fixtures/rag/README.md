...
- **맵 검사만으로는 부족합니다.** `322065`(슈렐리아의 군가 사본)나 `663193`(협곡에 없는 아이템)처럼
  ID 가 4자리를 넘는 모드 전용 사본도 `maps["11"]` 이 참입니다. 그래서 4자리 이하 ID 만 담습니다.
  경기 기록(MATCH-V5)과 인게임 API 가 보고하는 아이템 ID 도 4자리입니다. 이 규칙으로 43개가 빠집니다.
