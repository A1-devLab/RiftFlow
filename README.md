...
- 박시영의 수집 모듈: Data Dragon과 최근 한국어 패치 노트를 SQLite로 저장·갱신
- 실제 DB → 공통 문서 형식 → 이찬영의 RAG 검색 연결
- 시작 화면, 자료 검색·상세·출처, 질문 화면, 연결·신청 안내
- 키 없이 사용할 수 있는 고정 버전 예시 10건 (최신 게임 정보 아님)
- 선택적으로 Gemini 답변 요청. 기본은 로컬 근거 검색만 수행
- 업데이트·질문 처리를 백그라운드에서 실행해 UI 응답 유지

전적 조회·분석, 롤 로그인 감지, 룬 자동 적용, 실시간 아이템 추천은 아직 구현하지 않았습니다.
...
4. 연결 및 신청 → 공식 게임 자료 수집/업데이트를 누릅니다.
