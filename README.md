...
- 전적 대시보드(경기 날짜·상세 통계), 자료 검색·상세·출처, 질문 화면, 설정·데이터 관리
- 롤 클라이언트 로그인 자동 감지와 로그인 계정의 랭크·최근 20경기 조회
- 픽창 화면: 확정 픽·밴과 맞라인 상대 자동 표시, 픽창 전용 질문·프롬프트, 개인 상성·최근 사용 룬 기록
- 인게임 화면: 10명 실시간 스코어보드·아이템·팀별 추정 골드, 스코어보드와 공식 아이템 자료를 근거로 한 질문·아이템 추천
- 키 없이 사용할 수 있는 고정 버전 예시 10건 (최신 게임 정보 아님)
- 선택적으로 Gemini 답변 요청. 기본은 로컬 근거 검색만 수행
- 업데이트·질문 처리를 백그라운드에서 실행해 UI 응답 유지

룬 자동 적용은 아직 구현하지 않았습니다.
데스크톱은 롤 클라이언트 상태를 인식해 픽창 → 인게임 화면으로 자동 전환하고, 게임이 끝나면 전적 화면으로 돌아갑니다.
인게임 골드는 보유 아이템 가격 합계 추정치이며, 시야·상대 주문 쿨타임·오브젝트 타이머는 확인할 수 없습니다.
픽창의 개인 상성 기록은 로그인 계정에서 조회된 협곡 경기의 맞라인 상대만 집계하며 전체 플레이어 승률이 아닙니다.
...
`1. before game`은 챔피언, 상대 챔피언, 딜교환·라인전 성향을 받아 게임 전 전용 프롬프트로 답합니다. `2. in game`은 게임 중이면 실시간 스코어보드를 근거로, 아니면 게임 중 전용 프롬프트로 답합니다. `3. after game`은 결과 화면 수치로 경기 후 분석을 요청하는 개발용 흐름이며 데스크톱 화면은 없습니다. `4. out game`은 현재 메타와 패치 정보를 묻는 전용 공간입니다.
...
현재 177개 오프라인 테스트와 GUI 작업 완료 검증을 수행합니다. 패키지를 설치하지 않았다면 `PYTHONPATH=src`를 붙여 실행하세요. 실제 Gemini 호출과 Windows 환경은 별도 검증이 필요합니다.

## 협업 문서

- `docs/interfaces.md`: 실제 DB/RAG 연결 규격
- `docs/game-phases.md`: 단계별 폴더, 단계별 프롬프트 수정 위치와 데스크톱 단계 전환
