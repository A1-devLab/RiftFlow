...
- 동기 함수다. 결과가 올 때까지 부른 쪽이 멈춘다.
  Gemini 호출은 최악이면 125초다 (제한시간 60초 x 2번 시도 + 재시도 대기 5초).
...
- 오류 표현: 근거 자료 받기와 모델 호출에서 난 오류만 잡아 status 와 error 로 돌려준다.
  ...
  호출 한도 초과(429)는 message 에서 하루 한도와 분당 한도를 구분해 안내하지만,
  retry_after 와 한도 ID 는 결과에 담기지 않고 대화 기록의 error_detail 에만 남는다.
