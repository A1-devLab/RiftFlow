...
전체 앱을 설치하려면 저장소 루트에서 `python -m pip install -e .`를 실행하세요. 이 경우 GUI 등 프로젝트 의존성도 설치됩니다. knowledge 모듈만 사용한다면 표준 라이브러리만으로 실행할 수 있습니다.
...
- 패치 본문과 기본 챔피언·아이템·룬 섹션의 변경 전후 수치를 저장합니다. 분류 규칙으로 판단하기 어려운 항목은 unknown/adjusted로 남깁니다. 상세 범위는 아래 Out game 절을 참고하세요.
...
## Out game 구조화 데이터 (2026-09-20)

`update`는 정적 데이터와 패치 원문을 수집하고, 패치의 챔피언·아이템·룬 변경을 구조화합니다.
자세한 DB 열·메타데이터·파싱 범위는 [인터페이스 문서](../../docs/interfaces.md#out-game-db-구현-2026-09-20)를 참고하세요.
...
표본 경기 수는 정적 자료에 해당하지 않으므로 null입니다. 버프/너프 불명확 항목은
unknown/adjusted로 보존합니다. 라이엇 API 키나 LLM API는 필요하지 않습니다.
